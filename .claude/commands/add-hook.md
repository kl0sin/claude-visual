   ...
   "EventName": [
     {
       "hooks": [
         {
           "type": "command",
           "command": "INPUT=$(cat) && printf '%s' \"$INPUT\" | jq -c '. + {event_type: \"EventName\", session_id: (.session_id // env.CLAUDE_SESSION_ID)}' | curl -s -X POST -H 'Content-Type: application/json' -d @- http://localhost:3200/api/events > /dev/null 2>&1"
         }
       ]
     }
   ]
